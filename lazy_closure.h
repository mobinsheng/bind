 ...
 * }
*/
#ifndef LAZY_CLOSURE_H_
#define LAZY_CLOSURE_H_

#include "bind/bind.h"
#include "bind/closure.h"
...
public:
    ...
    Closure(std::shared_ptr<lazy::AsyncClosure> c):closure_(c){
...
private:
    std::shared_ptr<lazy::AsyncClosure> closure_;
};

template <class FunctorT>
Closure CreateClosure(FunctorT&& functor){
    std::shared_ptr<lazy::AsyncClosure> ac(new lazy::FireAndForgetAsyncClosure<FunctorT>(std::forward<FunctorT>(functor)));
    Closure closure(ac);
    return closure;
}

}
#endif // LAZY_CLOSURE_H_
